...
Class.pt.space = function(html){
    ...
    '<iframe allowtransparency="true" id="'+ dom.ifr +''+ times +'" name="'+ dom.ifr +''+ times +'" onload="$(this).removeClass(\'xubox_load\');" style="left:0px;" class="'+ dom.ifr +'" frameborder="0" src="' + config.iframe.src + '"></iframe>',				
    ...
    +'" class="'+ dom.lay +'">'	
    + '<div style="background-color:'+ config.bgcolor +'; z-index:'+ zIndex +'" class="xubox_main"><input type="hidden" id="'+ dom.ifr +'data'+ times +'" value="'+config.data+'"/>'
    + frame[config.type]
    + title
    + closebtn
...
Class.pt.openLayer = function(){
    ...
    //得到当前iframe层的索引，子iframe时使用--zy
    layer.getFrameData = function(name){
        var jj = $(name ? '#'+ name : '.'+ dom.ifr).parents('.'+dom.lay).attr('times');
        return $('#'+dom.ifr +'data'+jj).val();
    };
