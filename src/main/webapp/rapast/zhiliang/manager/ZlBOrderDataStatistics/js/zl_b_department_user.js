...
function queryOrderDataForMapBar(id) {
	$.ajax({
		url:getRequestUrl("/ZlBDepartmentUserController/queryOrderDataForMapBarByper.json"),
		dataType:"json",
		data:{"zlBDepartmentUser.id":id},
		success:function(data){
			if(data.zlBDepartmentUserList.length>0){
				for (var i=0,len=data.zlBDepartmentUserList.length;i<len;i++) {
					tradernum[i]={name:"["+data.zlBDepartmentUserList[i].departmentname+"]"+data.zlBDepartmentUserList[i].departmentuser,value:data.zlBDepartmentUserList[i].newsnum}
					cityarea[i]=data.zlBDepartmentUserList[i].departmentuser;
					ordersummoneyforbar[i]={name:"["+data.zlBDepartmentUserList[i].departmentname+"]"+data.zlBDepartmentUserList[i].departmentuser,value:data.zlBDepartmentUserList[i].newsnum}
				}
			}
			initMainAreaBar('个人投稿数量统计',cityarea,tradernum);
			selectTraderDistributeProvinceEchar('个人投稿数量统计',cityarea,ordersummoneyforbar);
