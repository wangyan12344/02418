...
function queryDataList(){
	...
	var parameterData = {
			url:getRequestUrl("/ZlBMagazineController/queryZlBMagazinePageList.json"),
...
function addCarBrandFun(){
	var page = "/ZlBMagazineController/add.do";
	pageForward(page);
}
...
function updateHandle(id){
	var page = "/ZlBMagazineController/update.do?opid="+id;
	pageForward(page);
}
...
function deleteHandle(id){
	...
	layer.confirm(
			...
			function(index){
				$.ajax({
					url:getRequestUrl("/ZlBMagazineController/deleteZlBMagazine.json"),
					dataType:"json",
					data:{"zlBMagazine.id":id},
...
function exportExcel(){
	...
	checkval();
	if (rowname!='') {
	location.href=path+"/ZlBMagazineController/exportProject.json?etime="+etime+"&stime="+stime+"&rowname="+encodeURI(encodeURI(rowname));
	} else {
