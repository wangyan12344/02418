...
public class ExportExcelUtil<T> {
	public String[] exportExcelHeader(Class<T> clz){
	    ...
	    public void exportExcel(String[] headerTitle,String[] headers, Collection<T> dataset,
	            OutputStream out, String newFileName,HttpServletResponse response) {
	        exportExcel("导出EXCEL文档",headerTitle, headers, dataset, out, null,newFileName,response);
	    }
	    ...
	    public void exportExcel(String title,String[] headerTitle, String[] headers,
	            Collection<T> dataset, OutputStream out, String pattern,String newFileName,HttpServletResponse response) {
	        // 声明一个工作薄
	            ...
	            for (int i = 0; i <= 4; i++) {
	                ...
	                try {
						generateSheet(list, style, style2, workbook, pattern,headerTitle, headers,
						        title + "_" + (i + 1),newFileName,response);
					} catch (Exception e) {
						e.printStackTrace();
					}
	            }
	    ...
	    public void generateSheet(Collection<T> dataset, HSSFCellStyle style,
	          ...
	          String[] headerTitle,String[] headers, String title,String newFileName,HttpServletResponse response) throws Exception {
	        // 生成一个表格
...
//	        comment.setAuthor("author");
	        ...
	        response.setContentType("application/ms-excel;");  
			   response.setHeader("Content-disposition", "attachment;filename="  
			        + new String(newFileName.getBytes("gb2312"), "ISO-8859-1") + ".xls");
}
